...
class OneDriveConnection(BaseConnection):
    ...
    def connect(self):
        ...
        if token_str:
            ...
            self.api.auth_get_token()
            keychain.set_password('onedrive', self.app_key, pickle.dumps((self.api.auth_access_token, self.api.auth_code, self.api.auth_refresh_token)))
...
class GoogleDriveConnections(BaseConnection):
    ...
    def write(self, file_name_path, contents):
        file = self.api.CreateFile({"title":os.path.basename(file_name_path), "mimeType":"text/plain"})
    ...
    def mk_dir(self, path):
        file = self.api.CreateFile({"title":path, "mimeType":"application/vnd.google-apps.folder"})
        file.Upload()
        self.file_ids[file["title"]] = ["FILE", file["id"]]
        return file
