...
def preview(sender):
    ...
    try:
        i = console.alert(
            "Launching Server", "Use Global Server?", "Yes", "No")
...
class TextEditorView(ui.View):
        ...
        if replacement in keys:
            ...
            if range[0] > 0:
                ...
                if last in bracket_open:
                    ...
                    self.get_tabs_for_line(
                        textview.text, textview.selected_range)
            return True

    def textview_did_change(self, textview):
        if len(textview.text) > len(self.old_text):
            if self.superview.fileManager:
                name = self.pagecontrol.segments[
    ...
    def get_tabs_for_line(self, text, textrange):
        ...
        x = textrange[1] - 1 if textrange[1] - \
            1 < len(text) and textrange[1] - 1 > 0 else len(text) - 1
        while True:
            # print x, len(text)
            if not x < len(text):
                break
    ...
    def comment_line(self):
        ...
        frange = [0, 0]
        x = srange[1] - 1 if srange[1] - \
            1 < len(text) and srange[1] - 1 > 0 else len(text) - 1
        while True:
            # print x, len(text)
            if not x < len(text):
                break
